...
struct AesCipherParams {
    enum class CryptoOperaTionType { DECRYPT = 0, ENCRYPT = 1 };
...
CryptoGuardCtx::PImpl::PImpl() : chiper_ctx_(EVP_CIPHER_CTX_new()), md_ctx_(EVP_MD_CTX_new()) {
    if (!chiper_ctx_)
...
std::string CryptoGuardCtx::PImpl::CalculateChecksum(std::istream &inStream) {
    ...
    while (inStream) {
        ...
        if (!EVP_DigestUpdate(md_ctx_.get(), inBuf.data(), static_cast<int>(bytesRead)))
    ...
    for (unsigned int i = 0; i < hashLen; ++i)
        ...
        oss << std::setw(2) << static_cast<unsigned int>(hash[i]);  // not char
...
void CryptoGuardCtx::PImpl::ApplyCryptoOperation(std::istream &inStream, std::ostream &outStream,
    ...
    while (inStream) {
        ...
        if (!EVP_CipherUpdate(chiper_ctx_.get(), outBuf.data(), &outLen, inBuf.data(), static_cast<int>(bytesRead)))
    ...
    if (!EVP_CipherFinal_ex(chiper_ctx_.get(), outBuf.data(), &outLen))
        throw std::runtime_error("EVP_CipherFinal_ex failed: " + getLastOpenSSLError());
...
std::string CryptoGuardCtx::CalculateChecksum(std::istream &inStream) { return impl_->CalculateChecksum(inStream); }
